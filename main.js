...
const screens = {
    ...
    typesPreview: document.getElementById('typesPreview')
};
...
function renderTypesPreview() {
    ...
    const allTypes = Object.entries(TYPE_LIBRARY).map(([code, type]) => ({
        code,
        ...type
    }));

    typesGrid.innerHTML = allTypes.map(type => {
        const imageSrc = TYPE_IMAGES[type.code];
        return `
          <div class="type-card" data-code="${type.code}">
            <div class="type-card-image">
              ${imageSrc ? `<img src="${imageSrc}" alt="${type.code}" />` : '<div class="type-card-no-image">暂无图片</div>'}
            </div>
            <div class="type-card-content">
              <div class="type-card-code">${type.code}</div>
              <div class="type-card-name">${type.cn}</div>
              <div class="type-card-intro">${type.intro}</div>
            </div>
    ...
    typesGrid.querySelectorAll('.type-card').forEach(card => {
...
document.getElementById('previewTypesBtn').addEventListener('click', () => {
    renderTypesPreview();
    showScreen('typesPreview');
});
document.getElementById('backFromTypesBtn').addEventListener('click', () => showScreen('intro'));
document.getElementById('startFromTypesBtn').addEventListener('click', () => startTest(false));